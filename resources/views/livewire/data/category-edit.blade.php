<div>
    ...
    <form wire:submit='update' >
        <div class="flex ">
            <div class="flex-initial w-6/12  ms-80">
                <div class="mb-6">
                    ...
                    @error('name') <span class="text-red-700 error">{{ $message }}</span> @enderror
                </div>

                <div class="mb-6">
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                        @if($categoryImg && $media = $categoryImg->getMedia()->last())
                    ...
                    @error('photo') <span class="text-red-700 error">{{ $message }}</span> @enderror
                </div>

                <div class="mb-6">

                    <label for="default-checkbox" class="block mb-2 text-lg font-medium text-gray-900">Status</label>
                    <input wire:model='status' id="default-checkbox" type="checkbox" value="0"
                    {{ $this->status === 1 ? 'checked' : '' }}
        ...
        <button type="submit"
            class="ms-80 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update</button>
    </form>
</div>
